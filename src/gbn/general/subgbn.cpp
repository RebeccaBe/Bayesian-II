...
namespace {
	std::pair<std::vector<Port>,std::vector<Port>> build_inputs_outputs_for_vertices(const GBN& gbn, std::vector<Vertex> inside_vertices)
	{
		...
		std::vector<std::pair<Port, Port>> input_port_helper; //saves the first target Port of a new input

		bool only_diagonal_matrices = true;
		...
		for(auto v : inside_vertices)
		{
			...
			for(auto e : boost::make_iterator_range(boost::in_edges(v,g)))
			{
				...
				if(!is_in(u,inside_vertex_set)) {
					...
					input_port_helper.push_back(std::pair<Port, Port> ({u, port_from(e, g)}, {v, port_to(e, g)}));
				}
			}
		...
		//check, if there are only diagonal matrices and the same number of input and output vertices
		//otherwise, there won't be a diagonal matrix, if evaluated and this next computation is useless.
		if(only_diagonal_matrices && (input_port_set.size() == output_port_set.size())) {
            std::vector<Port> output_ports_vec, input_ports_vec;

            for(auto port_pair : input_port_helper) {

                Port deep_target = port_pair.first;
                while(!is_in(deep_target, output_port_set)) {
					for (auto e : boost::make_iterator_range(boost::out_edges(deep_target.first, g))) {

					    Port port_helper = {boost::target(e, g), port_to(e, g)};

						if((port_pair.second == port_helper) || (deep_target != port_pair.first)) {
							if (port_from(e, g) == deep_target.second) {
								deep_target = port_helper;
								break;
							}
						}
					}
                }
                input_ports_vec.push_back(port_pair.first);
                output_ports_vec.push_back(deep_target);
            }

            return {input_ports_vec, output_ports_vec};

        } else {

            return {std::vector<Port>(input_port_set.begin(), input_port_set.end()),
