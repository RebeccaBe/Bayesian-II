...
std::string matrices_sizes(GBN gbn) {
    auto g = gbn.graph;
    std::string sizes;
    ...
    for(auto v : all_vertices(gbn))
        if(type(v, g) != OUTPUT) {
            ...
            switch(m->type) {
                case DIAGONAL:
                case F:
                    sizes += std::to_string(m->n); sizes += ";";
                    break;
                default:
                    sizes += std::to_string(m->n + m->m); sizes += ";";
            }
        }
    return sizes;
}
...
int main(int argc, const char** argv)
{
    ...
    for(std::size_t n_places = cn_params.N_MIN_PLACES; n_places <= cn_params.N_MAX_PLACES; n_places++)
	{
		...
		for(std::size_t i_run = 0; i_run < cn_params.N_RUNS; i_run++)
		{
			...
			evaluate_specific_place(0, gbn, cn_params.EVALUATION_TYPE);
            auto end_time = std::chrono::steady_clock::now();

			double diff_milliseconds = std::chrono::duration<double, std::milli>(end_time-start_time).count();

			csv_file << n_places << ";" << diff_milliseconds << ";" << matrices_sizes(gbn) << std::endl;
		}
	}
