...
int main(int argc, const char** argv)
{
	...
	options.add_options()
		...
		("n-max-places", "", cxxopts::value<std::size_t>()->default_value("10"))

		("n-runs", "", cxxopts::value<std::size_t>()->default_value("5"))
		("n-transitions-per-run", "", cxxopts::value<std::size_t>()->default_value("5"))

		("percent-min-tokens", "", cxxopts::value<double>()->default_value("0"))
		("percent-max-tokens", "", cxxopts::value<double>()->default_value("0"))
		...
		("export-name", "", cxxopts::value<std::string>()->default_value("evaluated"))
		("detailed", "")

		("free-choice", "")
		("comparison-value", "Not needed here.", cxxopts::value<std::size_t>()->default_value("1"))
		;
	...
	bool is_free_choice = params.count("free-choice") > 0;
	std::string file_addition = "";
	if (is_free_choice)
        file_addition = "_fc";
	...
	std::ofstream csv_file(params["export-name"].as<std::string>()+file_addition+".csv");

	if(is_free_choice)
		csv_file << "free choice net" << std::endl;
	csv_file << "n_places;milliseconds_old;milliseconds_improved" << std::endl;
	for(std::size_t n_places = cn_params.N_MIN_PLACES; n_places <= cn_params.N_MAX_PLACES; n_places++)
	{
		...
		for(std::size_t i_run = 0; i_run < cn_params.N_RUNS; i_run++)
		{
			...
			auto start_time_new = std::chrono::steady_clock::now();
			improved::evaluate(gbn);
			auto end_time_new = std::chrono::steady_clock::now();
			double diff_milliseconds_new = std::chrono::duration<double, std::milli>(end_time_new-start_time_new).count();

			if(is_detailed)
				std::cout << "evaluated with improved function." << std::endl;

			auto start_time_old = std::chrono::steady_clock::now();
			old::evaluate(gbn);
			auto end_time_old = std::chrono::steady_clock::now();
			double diff_milliseconds_old = std::chrono::duration<double, std::milli>(end_time_old-start_time_old).count();

			if(is_detailed)
				std::cout << "evaluated with old function." << std::endl;

			csv_file << n_places << ";" << diff_milliseconds_old << ";" << diff_milliseconds_new << std::endl;
		}
	}
