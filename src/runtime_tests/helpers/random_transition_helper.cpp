...
RandomTransitionHelper::RandomTransitionHelper(const CN& cn, Type type, double p_success, std::size_t max_trans)
	...
	uniform_0_1(0,1),
	valid_transitions(get_n_valid_transitions(cn))
{
...
std::vector<std::pair<std::size_t, double>> RandomTransitionHelper::next_p(std::mt19937& mt) {
	...
	bool valid = false;
	for(auto i_transition : chosen_transitions) {
		auto transition = cn.transitions[i_transition];
		if(check_pre_condition(transition, cn.m)) valid = true;
	}

	if(!valid && valid_transitions > 0) return next_p(mt);
